...
class OptimalEstimationProblem():
    ...
    function, and (optionally) a set of constraints on the state and/or
    inputs along the trajectory (and at the terminal time).  This class
    sets up an optimization over the initial state and disturbances at each
    point in time, using the integral and terminal costs as well as the
    trajectory constraints.  The `compute_estimate` method solves the
    underling optimization problem using :func:`scipy.optimize.minimize`.

    The `_cost_function` method computes the "cost" (negative of the log
    likelihood) of the estimated trajectory generated by the proposed
    initial estimated state, the disturbances and noise, and the measured
    output.  It does this by calling a user-defined function for the
    integral_cost given the current estimated states, inputs, and measured
    outputs at each point along the trajectory and then adding the value of
    a user-defined terminal cost at the initial point in the trajectory.

    The `_constraint_function` method evaluates the constraint functions
    along the trajectory generated by the proposed estimate and
    disturbances.  As in the case of the cost function, the constraints are
    evaluated at the estimated state, inputs, and measured outputs along
    each point on the trajectory.  This information is compared against the
    constraint upper and lower bounds.  The constraint function is
    processed in the class initializer, so that it only needs to be
    computed once.
    ...
    def __init__(
            ...
            trajectory_constraints=None, **kwargs):
        """Set up an optimal control problem."""
        # Save the basic information for use later
        self.system = sys
        self.timepts = timepts
    ...
    def _cost_function(self, xvec):
        ...
        if ct.isctime(self.system):
            ...
            # TODO: make sure last point is properly accounted for
            cost = sum(map(
    ...
    def _collocation_constraint(self, xvec):
        ...
        if self.system.isctime():
            ...
            # TODO: vectorize
            fk = self.system._rhs(
                self.timepts[0], xhat[:, 0], inputs[:, 0])
            for i, t in enumerate(self.timepts[:-1]):
                # From M. Kelly, SIAM Review (2017), equation (3.2), i = k+1
                # x[k+1] - x[k] = 0.5 hk (f(x[k+1], u[k+1] + f(x[k], u[k]))
                fkp1 = self.system._rhs(t, xhat[:, i+1], inputs[:, i+1])
            ...
            for i, t in enumerate(self.timepts[:-1]):
                ...
                self.colloc_vals[:, i] = xhat[:, i+1] - \
                    self.system._rhs(t, xhat[:, i], inputs[:, i])
    ...
    def _compute_states_inputs(self, xvec):
        ...
        yhat = np.vstack([
            self.system._out(self.timepts[i], xhat[:, i],
                              np.hstack([self.u[:, i], v[:, i]]))
    ...
    def compute_estimate(
        ...
        ----------
        Y, U : 2D array
            Measured outputs and applied inputs at each time point.
        X0 : 1D array
        ...
        self.ndisturbances = self.system.ninputs - self.u.shape[0]
        assert self.ndisturbances > 0
    ...
    def create_mhe_iosystem(
        ...
        ----------
        sys : InputOutputSystem
            I/O system for which the estimator will be computed.

        nd : int
            Number of inputs that are disturbance (versus control) inputs.
            The current implementation assumes that the inputs to `sys` are
            the control inputs followed by `nd` disturbance inputs.
        ...
        if isinstance(sensor_labels, str):
            # Generate labels using the argument as a format string
            sensor_labels = [sensor_labels.format(i=i)
                             for i in range(self.system.noutputs - nd)]
        ...
        def _mhe_update(t, xvec, uvec, params={}):
            ...
            off, U = _xvec_next(xvec, off, self.system.ninputs - nd)
            off, V = _xvec_next(xvec, off, nd)
        ...
        return ct.NonlinearIOSystem(
            ...
            inputs=sensor_labels + self.system.input_labels,
            outputs=output_labels, dt=self.system.dt)
